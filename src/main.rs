...
//!
//! Este módulo contém a função principal para iniciar o tracker ou um peer.
mod peer;
mod tracker;
mod chat;
...
async fn main() {
    ...
    if mode == "tracker" {
        iniciar_tracker().await;
    } else if mode == "peer" {
        iniciar_peer().await;
    }
}

/// Inicia o tracker na porta 6881.
async fn iniciar_tracker() {
...
/// Inicia um peer, solicita o nome do peer e o arquivo a ser compartilhado.
async fn iniciar_peer() {
    ...
    tokio::spawn(async move {
        if let Err(e) = message_receiver(receiver).await {
            eprintln!("Erro ao receber mensagens: {}", e);
        }
    });
...
async fn interface_comandos(peer: Arc<Peer>, peer_name: String) {
